...
static int chrometf_json_fill_event(struct convert *c,
				struct perf_sample *sample,
				struct evsel *evsel)
{
	...
	jsonw_uint_field(cw->writer, "pid", sample->pid);
	jsonw_uint_field(cw->writer, "tid", sample->tid);
	jsonw_float_field_fmt(cw->writer, "ts", "%f", (double)sample->time / 1000.0);
				...
				struct machine *machine __maybe_unused)
{
	...
	return chrometf_json_fill_event(c, sample, evsel);
}

#define __NON_SAMPLE_SET_FIELD(_name, _type, _f)   \
do { \
    jsonw_##_type##_field(cw->writer, #_f, _event->_name._f);\
} while (0)
...
{                                                               \
        ...
        c->events_size += _event->header.size;                  \
                                                                \
        jsonw_start_object(cw->writer);                         \
		jsonw_string_field(cw->writer, "name", #_name);         \
        jsonw_string_field(cw->writer, "cat", "non-sample");    \
        jsonw_string_field(cw->writer, "ph", "I");    \
        jsonw_uint_field(cw->writer, "pid", sample->pid);       \
        jsonw_uint_field(cw->writer, "tid", sample->tid);       \
        jsonw_float_field_fmt(cw->writer, "ts", "%f", (double)sample->time / 1000.0);  \
...
__FUNC_PROCESS_NON_SAMPLE(fork,
        __NON_SAMPLE_SET_FIELD(fork, uint, ppid);
        __NON_SAMPLE_SET_FIELD(fork, uint, ptid);
        __NON_SAMPLE_SET_FIELD(fork, uint, time);
)
...
__FUNC_PROCESS_NON_SAMPLE(mmap,
        __NON_SAMPLE_SET_FIELD(mmap, uint_hex, start);
        __NON_SAMPLE_SET_FIELD(mmap, string, filename);
)
__FUNC_PROCESS_NON_SAMPLE(mmap2,
        __NON_SAMPLE_SET_FIELD(mmap2, uint_hex, start);
...
static int chrometf_writer__init(struct chrometf_writer *cw, const char *path)
{
	...
	cw->file = fopen(path, "w");
	if (cw->file == NULL)
		return -1;

	writer = jsonw_new(cw->file);
	if (!writer)
		goto err;
